...
export default class List extends NestedList  { 
    ...
    rendered () {
        ...
        // console.log('allEditorBlocks',allEditorBlocks)
        for (let block = 0; block < allEditorBlocks.length; block++) {
            ...
            if (self.querySelector('.cdx-nested-list')) {
                const uniqClass = Math.random().toString(36).slice(2);
                self.querySelector('.cdx-nested-list').classList.add('main-nested-list');
                self.querySelector('.cdx-nested-list') ? self.querySelector('.cdx-nested-list').classList.add(`u-${uniqClass}`) : '';
                if (this.data.html) {
                    self.querySelector(`.cdx-nested-list`).innerHTML = this.data.html.replaceAll('&lt;', '<').replaceAll('&gt;', '>').replaceAll('&#47;', '/');
                    let menus = self.querySelector('.cdx-nested-list').querySelectorAll('.editMenu');
                    let innerMenus = self.querySelector('.cdx-nested-list').querySelectorAll('.context_wrapper');
                    for (let innerMenu = 0; innerMenu < innerMenus.length; innerMenu++) {
                        innerMenus[innerMenu].remove();
                    }
                ...
                this.addContext(wrapperElement);
                ...
                close.addEventListener('click', (e) => {
                    self.querySelector('.cdx-nested-list').classList.contains('active-main-list') ? self.querySelector('.cdx-nested-list').classList.remove('active-main-list') : '';
                    for (let list = 0; list < this.lists.length; list++) {
                        this.lists[list].classList.contains('border') ? this.lists[list].classList.remove('border') : '';
                    }
                })
                config.addEventListener('click', (e) => {
                    self.querySelector('.cdx-nested-list').classList.add('active-main-list');
                    for (let list = 0; list < this.lists.length; list++) {
                        this.lists[list].classList.add('border');
                    }
                })
            }
        }
    ...
    addContext(wrapperElement) {
        for (let list = 0; list < this.lists.length; list++) {
            ...
            this.lists = wrapperElement.querySelectorAll('ul, ol');
            let listTypes = this.lists[list].querySelectorAll('.context_wrapper div');
            for (let type = 0; type < listTypes.length; type++) {
                ...
                listTypes[type].addEventListener('click', (e) => {
                    ...
                    this.lists[list].parentElement.append(newList);
                    this.lists[list].remove();
    ...
    save () {
        ...
        this.data.html = caption.innerHTML.replaceAll('<', '&lt;').replaceAll('>','&gt;').replaceAll('/', '&#47;');
        this.data.wrapperTag = caption.tagName.toLowerCase();
        this.rendered()
        return this.data;
    }
}