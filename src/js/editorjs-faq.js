export default class Faq {
    ...
    render() {
        ...
        if(this.data.saved_question){
            ...
            for(let q = 0; q < get_question.length; q++){
                ...
                question.innerHTML = /*html*/ `
                    <div class="question_flex_wrapper">
                        <h3 contenteditable="${this.readOnly ? "false" : "true"}" placeholder="question">${input_question_data}</h3>
                        <div class="arrow" ${this.readOnly ? '' : 'onclick="openFaq(event)"' }></div>
                    </div>
                `;
    
                let input_answer_data = this.data && get_answer[q] ? get_answer[q] : 'Answer';
                answer.innerHTML = /*html*/ `<p contenteditable="${this.readOnly ? "false" : "true"}" placeholder="answer">${input_answer_data}</p>`;
            ...
            newAnswer.innerHTML = /*html*/ `<p contenteditable="${this.readOnly ? "false" : "true"}" placeholder="answer">${input_answer_data}</p>`;
